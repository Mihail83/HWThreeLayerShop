...
namespace HWThreeLayerShop.DAL.Repositories
{
    public class ShopContextRepository : IRepository
    {
        ...
        public List<GoodDTO> GetGoodDTOsByCataligId(int id)
        {
            ...
            var listGoodsByCatalogId = _context.Goods.Include("Catalogs").Where(p => p.Catalogs.Any(p=>p.Id==id));

            if (listGoodsByCatalogId == null) throw new NotFoundExeption("Товары отсутствуют");                            
